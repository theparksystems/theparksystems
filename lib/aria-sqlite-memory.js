...
export function askSqliteMemory(sessionId, text) {
  ...
  const answer = buildHumanAnswer(cleanText, context.retrievals, context.correlations);
...
function retrieveContext(db, sessionId, text, messageIndex) {
  ...
  const isMemoryQuestion = queryEntities.some((entity) =>
    ["sqlite", "memory", "supabase", "correlations"].includes(entity),
  );
...
function buildHumanAnswer(question, retrievals, correlations) {
  ...
  if (isGreeting(normalized) && !directMarketQuestion) {
    return "Hey, I'm here. You can ask me about the current crude oil, silver, or real estate calls, or just talk through what you're trying to understand.";
  }
  ...
  if (directMarketQuestion && (articleTexts.length || messageTexts.length)) {
    const firstReference = articleTexts[0] || messageTexts[0];
    const secondReference = articleTexts[1] || messageTexts[1];
    return cleanAnswer([
      `ARIA's read starts with the strongest retrieved reference: ${firstReference}`,
  ...
  if (messageTexts.length || correlationTerms.length) {
    return cleanAnswer([
      "I can work with that.",
      messageTexts[0] ? `The closest prior conversation context is: ${messageTexts[0]}` : "",
      correlationTerms.length
        ? `The local memory terms I would keep in view are ${correlationTerms.join(", ")}.`
        : "",
      "Tell me the decision you are trying to make, and I will connect the useful context without forcing it.",
    ]);
  }

  return "I can help with that. Tell me what you're trying to understand, and I'll answer normally first, then connect it back to the current ARIA slate only where it actually applies.";
}
...
function isGreeting(text) {
  return /^(hi|hello|hey|yo|sup|howdy)\b/.test(text) || text.includes("how are you") || text.includes("hows it going") || text.includes("how's it going");
}
