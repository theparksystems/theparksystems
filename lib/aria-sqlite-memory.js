...
function buildResponsePrompt(question, retrievals, correlations) {
  ...
  return [
    "Reprompt for ARIA:",
    `Answer the user's question naturally, like a human assistant. Use retrieved articles/messages and SQLite correlated memory when they directly apply. If no direct slate or memory data exists, respond normally and helpfully instead of giving a refusal or memory disclaimer.`,
    "",
    ...
    correlationLines,
    "",
    "Response intention: synthesize direct references first when available. When direct references are missing, answer conversationally, keep it concise, and gently steer back to the slate only if useful.",
  ].join("\n");
}
...
function buildHumanAnswer(question, retrievals, correlations) {
  ...
  return "I can help with that. I do not have a direct slate memory match for this yet, so I’ll answer normally: tell me what decision or market you’re weighing, and I’ll connect it back to the current ARIA slate where it applies.";
}
